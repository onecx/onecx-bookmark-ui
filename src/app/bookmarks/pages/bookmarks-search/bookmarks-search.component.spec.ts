...
describe('BookmarksSearchComponent', () => {
  ...
  beforeEach(async () => {
    ...
    bookmarksSearch = await TestbedHarnessEnvironment.harnessForFixture(
      fixture,
      BookmarksSearchHarness
    );
  });
  ...
  it('should dispatch searchButtonClicked action on page init', (done) => {
    store.scannedActions$
      .pipe(ofType(BookmarksSearchActions.searchTriggered))
      .subscribe(() => {
        done();
      });
  });
  ...
  it('should export csv data on export action click', async () => {
    ...
    expect(store.dispatch).toHaveBeenCalledWith(
      BookmarksSearchActions.exportButtonClicked()
    );
  });
