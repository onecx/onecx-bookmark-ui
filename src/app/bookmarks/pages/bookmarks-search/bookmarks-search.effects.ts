...
import { AppStateService, DialogState, ExportDataService, PortalDialogService, PortalMessageService } from '@onecx/portal-integration-angular';
...
} from './bookmarks-search.selectors';
...
export class BookmarksSearchEffects {
  ...
  exportData$ = createEffect(
    () => {
      return this.actions$.pipe(
        ...
        concatLatestFrom(() =>
          this.store.select(selectBookmarksSearchViewModel)
        ),
        map(([, viewModel]) => {
  ...
  refreshSearch$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(
        BookmarksSearchActions.updateBookmarksSucceeded,
        BookmarksSearchActions.deleteBookmarksSucceeded
      ),
      withLatestFrom(this.appStateService.currentWorkspace$.asObservable()),
      mergeMap(([, workspace]) => {
        return this.performSearch(workspace.workspaceName);
  ...
  editButtonClicked$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      concatLatestFrom(() =>
        this.store.select(bookmarksSearchSelectors.selectResults)
      ),
      map(([action, results]) => {
      ...
      switchMap((dialogResult) => {
        ...
        return this.bookmarksService
          .updateBookmark(itemToEditId, itemToEdit)
          .pipe(
            map(() => {
  ...
  deleteButtonClicked$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap(([dialogResult, itemToDelete]) => {
        ...
        return this.bookmarksService
          .deleteBookmarkById(itemToDelete.id ?? '')
          .pipe(
            map(() => {
              this.messageService.success({
